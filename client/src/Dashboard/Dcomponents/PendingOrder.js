...
const PendingOrder = () => {
    ...
    const { OrderStatus, getAllOrders, StatusFinder, PendingOrders } = useOrderContext();
    ...
    return (
        <div className='PendingOrder'>
            <div className="row text-center p-0">
                {PendingOrders === "" ? <h2 className="">No Orders</h2> : PendingOrders.map((item, index) => {

                    const { _id, instructions, method, total_price, shipping_fee, status, products, userInfo, date } = item;
                                    ...
                                    <div className="col-lg-12 col-xl-12" >
                                        ...
                                        <div className="row w-100 Status_Container m-0">
                                            <div className="col-lg-4 py-2">
                                                <button disabled={StatusFinder(status) >= 2 ? true : false} className={`btn btn-outline-warning shadow-0 rounded-1 ${StatusFinder(status) >= 2 ? "fst-italic" : ""}`} onClick={() => (OrderStatus(_id, "Shipped"))}>Shipped</button>
                                            </div>
                                            <div className="col-lg-4 py-2">
                                                <button disabled={StatusFinder(status) >= 3 ? true : false} className={`btn btn-outline-info shadow-0 rounded-1 ${StatusFinder(status) >= 3 ? "fst-italic" : ""}`} onClick={() => (OrderStatus(_id, "On The Way"))}>On The Way</button>
                                            </div>
                                            <div className="col-lg-4 py-2">
                                                <button disabled={StatusFinder(status) === 4 ? true : false} className={`btn btn-outline-success shadow-0 rounded-1 ${StatusFinder(status) === 4 ? "fst-italic" : ""}`} onClick={() => (OrderStatus(_id, "Completed"))}>Completed</button>
                                            </div>
                                        </div>
